...
// database backup/delete js 
$(document).ready(function () {
  function sendDatabaseAction(action) {
    const formData = $('#database-operation-form').serialize() + '&action=' + action;

    $.ajax({
      type: 'POST',
      url: base_url + from + '/database_backup/perform_action',
      data: formData,
      dataType: 'json',
      success: function (response) {
        if (response.status) {
  ...
  $('#backup-btn').on('click', function () {
    sendDatabaseAction('backup');
  });

  // Trigger delete action
  $('#delete-btn').on('click', function () {
    sendDatabaseAction('delete');
  });
});
