...
class Product extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $this->data['partners'] = $this->db->select(' pd.partner_name,u.id as partner_id,pd.id as partner_data_id')
                ...
                ->where(['ug.group_id' => '4','pd.status' => 1])
    ...
    public function add_product()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if(isset($_POST['edit_product_id']) && !empty($_POST['edit_product_id'])){
                // print
                    if($_POST['product_type'] == 'variable_product'){
                        foreach ($_POST['variant_price'] as $key => $value) {
                            if($_POST['variant_special_price'][$key] == $_POST['variant_price'][$key]){
                                $this->response['error'] = true;
                                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                                $this->response['csrfHash'] = $this->security->get_csrf_hash();
                                $this->response['message'] = 'Price and Special Price should not be same';
                                print_r(json_encode($this->response));
                                return false;
                            }
                        }
                    ...
                    if ($_POST['product_type'] == 'simple_product') {
                        if ($_POST['simple_price'] == $_POST['simple_special_price']) {
                            $this->response['error'] = true;
                            $this->response['csrfName'] = $this->security->get_csrf_token_name();
                            $this->response['csrfHash'] = $this->security->get_csrf_hash();
                            $this->response['message'] = 'Price and Special Price should not be same';
                ...
                if($_POST['product_type'] == 'variable_product'){
                    if($_POST['variant_price'] == $_POST['variant_special_price']){
                        $this->response['error'] = true;
                        $this->response['csrfName'] = $this->security->get_csrf_token_name();
                        $this->response['csrfHash'] = $this->security->get_csrf_hash();
                        $this->response['message'] = 'Price and Special Price should not be same';
    ...
    public function get_product_data()
    {
     ...
     public function get_active_product_data()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            $status = 1;
            return $this->product_model->get_product_details(null, null, $status);
        } else {
            redirect('admin/login', 'refresh');
        }
    }
