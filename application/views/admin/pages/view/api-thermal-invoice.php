            ...
            <tr>
                <td>
                    <div>Delivery Address<address>
                            ...
                            <?php if (isset($order_detls[0]['mobile'])) {
                                $mobile = $order_detls[0]['mobile'];
                                $masked_mobile = (defined('ALLOW_MODIFICATION') && ALLOW_MODIFICATION == 0) ?
                                    ((strlen($mobile) > 3) ? str_repeat("X", strlen($mobile) - 3) . substr($mobile, -3) : $mobile) :
                                    $mobile;
                                ?>
                                <strong><?= $masked_mobile; ?></strong><br>
                            <?php } ?>
                            
                            <?php if (isset($order_detls[0]['email'])) {
                                $email = $order_detls[0]['email'];
                                $masked_email = (defined('ALLOW_MODIFICATION') && ALLOW_MODIFICATION == 0) ?
                                    ((strlen($email) > 3) ? str_repeat("X", strlen($email) - 3) . substr($email, -3) : $email) :
                                    $email;
                                ?>
                                <strong><?= $masked_email; ?></strong><br>
                            <?php } ?>
