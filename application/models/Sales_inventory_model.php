...
class Sales_inventory_model extends CI_Model
{
    public function get_sales_inventory_list(
        ...
        if (isset($_GET['partner_id']) && $_GET['partner_id'] != null) {
        ...
        $search_res = $this->db->select('oi.id,oi.product_variant_id,pd.partner_name as partner_name ,p.name, SUM(oi.quantity) AS qty,(p.availability OR pv.availability ) AS availability,(CASE WHEN (p.stock OR pv.stock) THEN p.stock ELSE pv.stock END) AS stock')
            ->join('product_variants pv', 'pv.id=oi.product_variant_id', 'left')
            ->join('products p', 'p.id=pv.product_id', 'left')
            ->join('partner_data pd', 'pd.user_id=p.partner_id', 'left')
            ->where('p.name !=', '');
        ...
        foreach ($user_details as $row) {
                ...
                $tempRow['partner_name'] = (isset($row['partner_name']) && $row['partner_name'] != '') ?  stripslashes($row['partner_name']) : "-";
                $tempRow['stock'] = $stock;
