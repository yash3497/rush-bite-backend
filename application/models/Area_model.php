...
class Area_model extends CI_Model
{
    ...
    function add_city($data)
    {
        ...
        if (isset($data['edit_city']) && !empty($data['edit_city']) && $data['edit_city'] != "") {
            ...
            if (isset($data['bordering_city_ids']) && !empty($data['bordering_city_ids'])) {
                $bordering_city_data = [
                    'city_id' => $data['edit_city'],
                    'bordering_city_id' => $data['bordering_city_ids']
                ];

                $bordering_city = fetch_details(['city_id' => $data['edit_city']], 'bordering_cities', '*');
                if (!empty($bordering_city)) {

                    $this->db->set($bordering_city_data)->where('id', $data['edit_city'])->update('bordering_cities');
                } else {
                    $this->db->insert('bordering_cities', $bordering_city_data);
                }
