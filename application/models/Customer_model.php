...
class Customer_model extends CI_Model
{
    ...
    public function get_customer_list($wallet_customer = NULL)
    {
        $offset = 0;
        $limit = 10;
        $sort = 'u.id';
        ...
        $search_res = $this->db->select(' u.*,DATE(u.created_at) as created_at,a.area as area_name,c.name as city_name')->join('cities c', 'u.city=c.id', 'left')->join('addresses a', 'u.id=a.user_id', 'left');;
        if (isset($multipleWhere) && !empty($multipleWhere)) {
